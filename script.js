...
class TwoFish{
    ...
    h(value, subKeyByte){
        ...
        if(subKeyByte.length < 4){
            q.shift();
            if(subKeyByte.length < 3) {
                q.shift();
            }
        }
        for (let i = 0, indexM = subKeyByte.length - 1; i <= subKeyByte.length; i++){
            ...
            for(let j = 0; j < 4; j++){
                ...
                let y = q[i][j] === 1 ? this.permutation(tempQ[j], q1) : this.permutation(tempQ[j], q0);
            ...
            if(i === subKeyByte.length){
                ...
                tempArray = this.multiplyVectorByMatrix(tempArray, MDS);
                let result = 0;
                for(let j = 0; j < 4; j++){
                    result += tempArray[j];
                }
    ...
    multiplyVectorByMatrix(vector, matrix) {
        ...
        if (vector.length !== rows) {
            console.error("Невозможно умножить вектор на матрицу: количество элементов вектора не соответствует количеству строк матрицы.");
            return;
        }
